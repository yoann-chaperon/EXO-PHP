<!--AJOUTE DES REQUETES DELETE INSERT UPDATE -->
<?php
    $connection = mysqli_connect("localhost", "yoann", "guizmo", "forum");
        ...
        if ($_GET['action']=='update' || $_GET['action']=='delete') {
            ...
            // envoyer la requete au serveur Mysql
        ...
        if (isset ($_POST['submit'])) {
            ...
            //tester le paramètre action pour création de la news en BDD:
            if ($_POST['action']=='insert') {
                //générere une requete de la création de news               
                $query = "INSERT INTO news (titre, contenu, membre_id)
                            VALUE ('".$_POST['titre']."',
                                    '".(addslashes($_POST['contenu']))."',
                                    '".$_POST['membre_id']."');";
                                             }
            elseif ($_POST['action']=='update') {
                //générer un requete de modification de news
                $query = "UPDATE news
                        SET titre='".$post['titre']."',
            ...
            elseif ($_POST['action']=='delete') {
                //générer un requete de suppression de new
                $query = "DELETE FROM news
                            WHERE id ='".$_POST['id']."';";
            }
            //echo $query // pour debobage en cas de "SYNTAX ERROR"
